FROM alpine:3.7
LABEL maintainer=jacob@sznajdman.com
...
COPY docker/util/* /usr/local/bin/
COPY docker/config/bashrc /root/.bashrc
COPY docker/patches /root/.patches
COPY docker/config/repositories /etc/apk/repositories
COPY docker/config/*.rsa.pub /etc/apk/keys/
...
COPY docker/config/jupyter /root/.jupyter/
COPY docker/config/ipydeps /root/.config/ipydeps/
...
RUN \
  ...
  min-pip3 notebook==5.2.2 numpy pandas jupyter ipywidgets==6.0.1 jupyter_dashboards pypki2 ipydeps ordo
...
ADD docker/kernels /usr/share/jupyter/kernels/
